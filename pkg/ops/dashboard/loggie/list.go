...
func NewLoggieListPanel(g *gui.Gui, state []AgentState) *ListPanel {
	...
	l.SetData()
	l.SetKeybinding(g)
	g.Nav.AddKeyBindingsNavWithKey(ListPanelName, "", gui.KeyEnter, gui.KeyQ, gui.KeyL)
...
func (l *ListPanel) SetData() {
	...
	if l.filterWord != "" {
		var filterStat []AgentState
		for _, s := range l.states {
			if !strings.Contains(s.PodName, l.filterWord) {
				continue
			}
...
func (l *ListPanel) SetKeybinding(g *gui.Gui) {
	l.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		switch event.Key() {
		case tcell.KeyEnter:
			switchToLoggieDetail(l.Table, l.gui, l.states, gui.LoggieListPage)
		}
		switch event.Rune() {
		case 'l':
			logs(l.Table, l.gui, l.states, gui.LoggieListPage)
		}
